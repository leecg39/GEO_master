...
const initial: SettingsState = { brandName: "", category: "", competitors: [], models: { openai: "gpt-4.1-mini", anthropic: "claude-sonnet-4-5", gemini: "gemini-2.5-flash", hyperclova: "HCX-DASH-002" }, repetitions: 3, modelWeights: { openai: .3, anthropic: .25, gemini: .2, hyperclova: .25 }, apiKeys: { openai: { configured: false, preview: null, error: false }, anthropic: { configured: false, preview: null, error: false }, gemini: { configured: false, preview: null, error: false }, hyperclova: { configured: false, preview: null, error: false } } };
...
export function SettingsClient() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setSettings(data.settings); setCompetitors(data.settings.competitors.join(", ")); setKeys({ openai: "", anthropic: "", gemini: "", hyperclova: "" }); setClearKeys([]); setMessage("설정이 안전하게 저장되었습니다.");
