...
import { Lightbulb, MapPin, Minus, Sparkles, TrendingDown, TrendingUp } from "lucide-react";
import { Badge, Card, EmptyState } from "@/components/ui";
import type { LocalBusinessBriefingData } from "@/lib/semforge/local-business/briefing";
...
export function LocalBusinessBriefing({
  campaignName,
  locationLabel,
  updatedAt,
  briefing,
  ...
  briefing: LocalBusinessBriefingData;
}) {
  if (!briefing.ready) {
    return (
      <Card className="mt-5 border-white/8">
        <div className="flex items-start gap-3">
          <span className="grid h-10 w-10 place-items-center rounded-xl bg-slate-800"><MapPin className="h-5 w-5 text-slate-400" /></span>
          <div>
            <h3 className="font-semibold text-white">{campaignName}</h3>
            <p className="mt-1 text-sm text-slate-400">{locationLabel} · 아직 Map Rank 데이터가 없습니다.</p>
            <p className="mt-2 text-xs text-slate-500">지역 키워드를 추가하고 Map Rank 수집을 실행하세요.</p>
          </div>
        </div>
      </Card>
    );
  ...
  return (
    <div className="mt-5 space-y-5">
      <Card className="border-violet-400/15 bg-gradient-to-br from-slate-900/90 to-violet-950/20">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <Sparkles className="h-4 w-4 text-violet-300" />
              <h3 className="text-lg font-semibold text-white">{campaignName} · Map Rank 분석</h3>
              <Badge tone="cyan">가시성 {briefing.visibility}%</Badge>
            </div>
            <p className="mt-2 text-sm text-slate-400">
              {locationLabel} · {updatedAt ? new Date(updatedAt).toLocaleString("ko-KR") : "—"} 수집
            </p>
          ...
          <div className="w-full max-w-xs shrink-0 rounded-xl border border-white/8 bg-slate-950/50 p-3">
            <VisibilityGauge value={briefing.visibility} label="Map Rank 가시성" />
            <p className="text-center text-sm font-semibold text-violet-300">
              Local Pack {briefing.localPackCount}/{briefing.totalKeywords} · 평균 {briefing.avgMapPosition ?? "—"}위
            </p>
          </div>
        </div>
      </Card>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          { label: "Local Pack", value: briefing.localPackCount, sub: "노출 키워드" },
          { label: "Map Top 3", value: briefing.top3Count, sub: "1–3위 키워드" },
          { label: "평균 Map 순위", value: briefing.avgMapPosition ?? "—", sub: "노출 기준" },
      ...
      <div className="grid gap-5 xl:grid-cols-3">
        <Card className="xl:col-span-2">
          <h4 className="mb-4 text-sm font-semibold text-white">Map 순위 구간</h4>
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <Card>
          <h4 className="mb-4 text-sm font-semibold text-white">5축 지역 SEO 레이더</h4>
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <Card>
          ...
          {briefing.recommendations.length === 0 ? (
            <EmptyState>현재 지역 SEO 상태가 양호합니다.</EmptyState>
          ) : (
      ...
      <Card>
        <h4 className="mb-4 text-sm font-semibold text-white">지역 키워드 Map 순위</h4>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[36rem] text-left text-sm">
            <thead>
              <tr className="border-b border-white/8 text-xs text-slate-500">
                <th className="pb-3 pr-4 font-medium">키워드</th>
                <th className="pb-3 pr-4 font-medium">Map</th>
                <th className="pb-3 pr-4 font-medium">Local Pack</th>
            ...
            <tbody>
              {briefing.keywords.map((item) => (
                <tr key={item.id} className="border-b border-white/5 text-slate-300">
                  ...
                  <td className="py-3 pr-4 font-semibold text-violet-300">{item.mapPosition ?? "—"}</td>
                  <td className="py-3 pr-4">{item.inLocalPack ? <Badge tone="good">노출</Badge> : <Badge>없음</Badge>}</td>
                  <td className="py-3 pr-4 text-slate-500">{item.previousMapPosition ?? "—"}</td>
