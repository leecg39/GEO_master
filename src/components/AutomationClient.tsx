...
import { AlertTriangle, CalendarClock, CircleDollarSign, Clock3, LoaderCircle, Pause, Pencil, Play, RefreshCw, RotateCcw, Save, Sparkles, Trash2, XCircle } from "lucide-react";
...
export function AutomationClient() {
  ...
  return <div aria-busy={Boolean(busy)}>
    ...
    <Card className="mt-6"><div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold text-white">예약 목록</h2><p className="mt-1 text-xs text-slate-500">중단 기간의 누락 슬롯은 한 건으로 합치며 대량 따라잡기를 실행하지 않습니다.</p></div><div className="flex flex-wrap gap-2"><Button type="button" variant="secondary" disabled={Boolean(busy)} onClick={() => void action({ action: "content.suggestFromShare", limit: 5 }, "미인용→GEO Blocks 초안")}><Sparkles className="h-4 w-4" />suggest_content_from_share</Button><Button type="button" variant="secondary" disabled={Boolean(busy)} onClick={() => void action({ action: "queue.process" }, "큐 1건 처리")}><Play className="h-4 w-4" />지금 큐 1건 처리</Button></div></div>
