...
const tabs: { id: StudioTool; label: string; description: string; icon: typeof FilePenLine }[] = [
  { id: "rewrite", label: "리라이팅", description: "5가지 패턴", icon: FilePenLine },
...
export function StudioClient() {
  const [action, setAction] = useState<StudioTool>("rewrite");
  ...
  const selectStored = useCallback((content: StudioContentResource | null) => {
    if (!content) return;
    const input = asRecord(content.input) ?? {};
    const output = asRecord(content.output);
    setAction(content.tool);
    ...
    setEntity({
      ...
      metric: asString(input.metric),
      url: asString(input.url),
      logo: asString(input.logo),
      sameAs,
    });
    if (typeof input.provider === "string" && ["openai", "anthropic", "gemini", "grok"].includes(input.provider)) {
      setProvider(input.provider as typeof provider);
    }
    setResult(output);
    setError(output ? "" : "저장된 스튜디오 결과 형식이 올바르지 않습니다.");
    resetRequest();
  }, []);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const data = await json<{ result: { output: Record<string, unknown> } }>(await fetch("/api/studio", {
        method: "POST",
  ...
  return <div>
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <Card>
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="text-sm">저장 제목 <span className="text-xs text-slate-600">(선택)</span><input className="mt-2" maxLength={120} value={title} onChange={(e) => { setTitle(e.target.value); resetRequest(); }} placeholder="예: 3월 리라이팅" /></label>
            <label className="text-sm">메모 <span className="text-xs text-slate-600">(선택)</span><input className="mt-2" maxLength={5000} value={notes} onChange={(e) => { setNotes(e.target.value); resetRequest(); }} placeholder="캠페인이나 담당자" /></label>
          </div>
          {action === "rewrite" && <>
            <label className="block text-sm">원문<textarea className="mt-2" rows={12} required value={text} onChange={(e) => { setText(e.target.value); resetRequest(); }} placeholder="리라이팅할 콘텐츠를 붙여넣으세요." /></label>
            <fieldset><legend className="mb-2 text-sm text-slate-300">적용 패턴</legend>
              <div className="flex flex-wrap gap-2">{patterns.map((pattern) => (
                <label key={pattern} className={`cursor-pointer rounded-lg border px-3 py-2 text-xs ${selectedPatterns.includes(pattern) ? "border-cyan-400/30 bg-cyan-400/10 text-cyan-300" : "border-white/8 text-slate-500"}`}>
                  <input className="sr-only" type="checkbox" checked={selectedPatterns.includes(pattern)} onChange={(e) => { setSelectedPatterns((items) => e.target.checked ? [...items, pattern] : items.filter((item) => item !== pattern)); resetRequest(); }} />{pattern}
                </label>
              ))}</div>
            </fieldset>
          </>}
          {(action === "intro" || action === "faq") && <>
          ...
          {action === "entity" && <div className="grid gap-4 sm:grid-cols-2">
            ...
            <label className="text-sm">핵심 가치<input className="mt-2" required value={entity.value} onChange={(e) => { setEntity((v) => ({ ...v, value: e.target.value })); resetRequest(); }} /></label>
            <label className="text-sm">카테고리<input className="mt-2" required value={entity.category} onChange={(e) => { setEntity((v) => ({ ...v, category: e.target.value })); resetRequest(); }} /></label>
            <label className="text-sm">수치·성과 <span className="text-slate-600">(선택)</span><input className="mt-2" value={entity.metric} onChange={(e) => { setEntity((v) => ({ ...v, metric: e.target.value })); resetRequest(); }} /></label>
            <label className="text-sm">공식 URL <span className="text-slate-600">(선택)</span><input className="mt-2" type="url" value={entity.url} onChange={(e) => { setEntity((v) => ({ ...v, url: e.target.value })); resetRequest(); }} /></label>
      ...
      <Card className="min-h-[32rem]">
        <div className="mb-4 flex items-center justify-between">
          <div><h2 className="font-semibold text-white">결과</h2><p className="mt-1 text-xs text-slate-500">실행 결과는 활성 프로젝트 이력에 저장됩니다.</p></div>
          {result && <Button type="button" variant="secondary" onClick={() => void copy()}>{copied ? <Check className="h-4 w-4" /> : <Clipboard className="h-4 w-4" />}{copied ? "복사됨" : "JSON 복사"}</Button>}
        </div>
        {result ? <div className="space-y-4">
          {"before" in result && <div className="rounded-xl border border-white/7 bg-slate-950/40 p-4"><Badge>Before</Badge><p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-500">{String(result.before)}</p></div>}
          {"after" in result && <div className="rounded-xl border border-cyan-400/15 bg-cyan-400/[0.04] p-4"><Badge tone="cyan">After</Badge><p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-200">{String(result.after)}</p></div>}
    ...
    <StudioHistoryPanel tool={action} onSelect={selectStored} />
  </div>;
}
